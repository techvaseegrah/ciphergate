...
const calculateWorkerProductivity = (productivityParameters) => {
    ...
    if (inPunch.time > workStart && inPunch.time !== outPunch.time) {
      ...
      if (isFirstSession) {
        ...
        // NEW: Special handling for first session after lunch
          ...
          // Cap at lunch start if punching in during lunch (already handled by 'if' above)
          // or just compare to start time.
          lateMinutes = inPunch.time - workStart;
        }
      } else {
      ...
      if (leaveInfo) {
        ...
        if (missedActiveProject) {
          ...
          if (isActuallyPaid) {
            ...
            // Unpaid leave on project day: no earnings, deduction tracked in project pool
            totalProjectDeductions += deductionAmount;
            const pid = missedActiveProject._id.toString();
            if (!projectBreakdownMap[pid]) {
              projectBreakdownMap[pid] = { grossEarned: 0, totalDeduction: 0, daysCount: 0 };
            }
            projectBreakdownMap[pid].grossEarned += 0;
        ...
        // PROJECT absent day: deduct from project earnings, NOT base salary
        // SAAS absent day: deduct from SaaS earnings (no gross earned)
        ...
        if (missedActiveProject) {
          // Absent project day: gross = project per-day value, deduction = perDaySalary * factor
  ...
  //   e.g. ₹50000 − ₹49863.61 = ₹136.40
  //
  // This is the correct SaaS take-home because it uses the FULL salary as the
  // starting point and subtracts all deductions, so it cannot be artificially
  // zeroed by the pool model's "only accumulate worked-day gross" limitation.
  const netBaseSalary = Math.max(0, originalSalary - totalSalaryDeduction);

  // ✅ Final salary — all penalties fully applied
  // netBaseSalary is the SaaS component (originalSalary − all deductions, clamped ≥ 0).
  // totalProjectSalary is the project pool component (clamped ≥ 0).
  // We do NOT add totalSaaSSalary here because it only covers "worked-day gross"
  // and gives ₹0 when absent deductions dominate, losing the residual balance.
  const finalSalary = netBaseSalary + totalProjectSalary;
  ...
  const finalSummary = {
    ...
    "Base Salary": formatCurrency(originalSalary),
